...
autoregControlInput(200,200)=0;
time=0:1000*numSwaps;

%unregModel=build.nCellUnregulatedSwapModel(N,unregControlInput,time);
autoregModel=build.twoCellAutoregulatedSwapModel(numSwaps,autoregControlInput,time);
AcademicFigure
...
for i=1:3
  index=mod(i-1,2)+1;
  X=[X,data.node{1}.state(index,L)];
  Y=[X,data.node{1}.state(index,L)];
  L=L+1000;
end
for i=1:length(X)
...
subplot(2,3,[1 2])
axis1=area(data.node{1}.time(1:L),U,'FaceColor','m');
axis1.EdgeAlpha=0;
tightLayout
...
s1=stairs(data.node{1}.time,data.node{1}.state(1,:)','LineWidth',2);
s1.Color=[0 0 1]*.8;
s2=stairs(data.node{1}.time,data.node{1}.state(2,:)','LineWidth',2);
s2.Color=[1 0 0]*.8;
