...
%valid [ka ga] = [.65 .0325] or [ka ga] = [.400 0.020]
  ...
  methods
    ...
    function model=autoregulatedModelWithUniformInput(obj,lightLevel,time)
      model=obj.makeModel;
      model.stoichMatrix=obj.stoichMatrix;
      model.parameters=[obj.ko obj.be obj.mu obj.ka obj.ga, lightLevel];
      model.rxnRate=@(t,x,p)[hill(x,p(1),p(2),p(3),p(4))+p(6) ; p(5)*x(1)];
      model.initialState=[0];
      model.time=obj.time;
      model.dims=obj.dims;
    end 
    function model=autoregulatedControlModelWithUniformControler(obj,lightLevel)
      model=ControlModel;
      model.stoichMatrix=obj.stoichMatrix;
      model.parameters=[obj.ko obj.be obj.mu obj.ka obj.ga, lightLevel];
      model.rxnRate=@(t,x,p)[hill(x,p(1),p(2),p(3),p(4))+p(6) ; p(5)*x(1)];
      ...
      model.time=obj.time;
      model.dims=obj.dims;
      model.controlInput=ones(obj.dims)*lightLevel;
    end 
