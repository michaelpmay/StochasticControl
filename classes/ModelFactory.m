...
%valid [ka ga] = [.65 .0325] or [ka ga] = [.400 0.020]
  ...
  methods
    ...
    function model=unregulatedModelWithExperimentalInput(obj)
      ...
      model.rxnRate=@(t,x,p)[p(1)+obj.ExperimentalInput(t,x,[p(3),p(4),p(5)]) ; p(2)*x(1)];
    ...
    function model=unregulatedModelWithFrequencyInput(obj,frequency,amplitude)
      ...
      model.rxnRate=@(t,x,p)[p(1)+obj.frequencyInput(t,x,[p(3) p(4)]) ; p(2)*x(1)];
      model.initialState=[0];
      model.time=obj.time;
    end
    ...
    function model=khammashFitModelNoGamma(obj)
      ...
      model.rxnRate=@(t,x,p)[p(1)+obj.ExperimentalInput(t,x,[p(2),p(3),p(4)]) ; .0203*x(1)];
      model.initialState=[0];
      model.time=obj.time;
    end
